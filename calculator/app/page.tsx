...
export default function Calculator() {
  ...
  // ── DETECTAR 911 Y REDIRIGIR A /api ────────────────────────────────────────
  useEffect(() => {
    if (display === "911") {
      router.push("/api");
    }
    ...
    <>
      <style>{`
        * { box-sizing: border-box; }
        ...
        .bg { position: fixed; inset: 0; background: radial-gradient(ellipse 60% 50% at 20% 30%, rgba(255,60,80,.12), transparent 70%), #0a0a0f; }
        .wrapper { min-height: 100vh; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 20px; position: relative; z-index: 1; }
        .hint { color: rgba(255,255,255,.5); font-size: 12px; display: flex; align-items: center; gap: 12px; }
        .hint-bar { width: 90px; height: 4px; background: rgba(255,255,255,.08); border-radius: 999px; overflow: hidden; }
        .hint-bar-fill { height: 100%; background: linear-gradient(90deg, #ff3c50, #ff8c00); transition: width .05s linear; }
        .calc { position: relative; width: 320px; background: rgba(20,20,28,.85); border-radius: 28px; overflow: hidden; backdrop-filter: blur(24px); border: 1px solid rgba(255,255,255,.06); box-shadow: 0 30px 80px rgba(0,0,0,.6); }
        .settings-btn { position: absolute; top: 14px; left: 14px; z-index: 5; width: 38px; height: 38px; border: none; border-radius: 12px; background: rgba(255,255,255,.06); color: white; cursor: pointer; }
        .display { min-height: 120px; padding: 28px 24px 18px; display: flex; flex-direction: column; justify-content: flex-end; align-items: flex-end; }
        .display-sub { color: rgba(255,255,255,.35); margin-bottom: 8px; }
        .display-main { color: white; font-size: 48px; font-weight: bold; }
        .grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 1px; background: rgba(255,255,255,.04); }
        .btn { border: none; height: 72px; color: white; cursor: pointer; font-size: 20px; background: rgba(255,255,255,.06); transition: transform .08s, filter .1s; }
        ...
        .alert-field { margin-bottom: 14px; padding: 14px; border-radius: 14px; background: rgba(255,255,255,.04); }
        .alert-label { color: rgba(255,255,255,.45); font-size: 11px; margin-bottom: 6px; text-transform: uppercase; }
        .alert-value { color: white; display: flex; align-items: center; gap: 6px; }
        .video-preview { width: 100%; border-radius: 16px; overflow: hidden; margin-top: 18px; background: black; }
        .video-preview video { width: 100%; display: block; }
        .alert-close { margin-top: 18px; width: 100%; height: 48px; border: none; border-radius: 14px; background: rgba(255,60,80,.18); color: #ff6473; cursor: pointer; font-size: 15px; font-weight: bold; }
      `}</style>
      ...
      <div className="wrapper">
        <div className="hint">
          <span>Hold SPACE for emergency</span>
        ...
        <div className="calc">
          ...
          <div className="display">
            <div className="display-sub">
              {stored && operator ? `${stored} ${operator}` : ""}
            </div>
      ...
      {showAlert && (
        <div className="overlay" onClick={() => setShowAlert(false)}>
          <div className="alert-box" onClick={e => e.stopPropagation()}>
            <div className="alert-title">🚨 Emergency Triggered</div>
            ...
            {isRecording && (
              <div className="recording">
                ...
                <strong>{recordingCountdown}s</strong>
              </div>
            )}

            <div className="alert-field">
              <div className="alert-label">Person at risk</div>
              <div className="alert-value">{ALERT_INFO.name}</div>
            </div>

            <div className="alert-field">
              <div className="alert-label">Location</div>
              <div className="alert-value">{ALERT_INFO.live_location}</div>
            </div>

            <div className="alert-field">
              <div className="alert-label">Offline mode</div>
              <div className="alert-value">
                {ALERT_INFO.offline === "yes" ? "🔴 Offline" : "🟢 Online"}
              </div>
            </div>

            <div className="alert-field">
              <div className="alert-label">Battery</div>
              <div
                className="alert-value"
                style={{
                  color: batteryLevel !== null && batteryLevel <= 20 ? "#ff5f5f" : "white",
                }}
              >
                {batteryLevel !== null ? (
